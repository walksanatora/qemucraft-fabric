...
public object Shaders {
    ...
    private fun loadShader(rm: ResourceManager, id: String): Int {
        val vshs = rm.getResource(Identifier(MOD_ID, "shaders/$id.vert")).get().inputStream.bufferedReader().readText()
        val fshs = rm.getResource(Identifier(MOD_ID, "shaders/$id.frag")).get().inputStream.bufferedReader().readText()
